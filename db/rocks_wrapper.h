...
class RocksWrapper
{
  public:
    ...
    std::unique_ptr<rocksdb::Iterator> seek_for_prev(rocksdb::ColumnFamilyHandle *family, const rocksdb::Slice &key,
                                                     bool asprefix)
    {
    ...
    std::unique_ptr<rocksdb::Iterator> seek_for_next(rocksdb::ColumnFamilyHandle *family, const rocksdb::Slice &key,
                                                     bool asprefix)
    {
        rocksdb::ReadOptions roption(false, true);
        roption.fill_cache = false;
    ...
    rocksdb::TransactionDB *get_db() { return _txn_db; }

    int32_t create_column_family(std::string cf_name);
    int32_t delete_column_family(std::string cf_name);

    rocksdb::Options get_options(rocksdb::ColumnFamilyHandle *family) { return _txn_db->GetOptions(family); }

    rocksdb::DBOptions get_db_options() { return _txn_db->GetDBOptions(); }

    rocksdb::Cache *get_cache() { return _cache; }
    const rocksdb::Snapshot *get_snapshot() { return _txn_db->GetSnapshot(); }
    void relase_snapshot(const rocksdb::Snapshot *snapshot) { _txn_db->ReleaseSnapshot(snapshot); }
    void close() { delete _txn_db; }
    bool is_any_stall()
    {
        uint64_t value = 0;
