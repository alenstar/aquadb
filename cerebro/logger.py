...
In order to also log to a file, just use `logger.logfile(..)`:
...
`setup_logger(..)`:
    ...
    logger = logger.setup_logger(logfile="/tmp/test.log")
...
COMMON_DEFAULT_LOGGER = "default"

# Attribute which all internal loggers carry
COMMON_INTERNAL_LOGGER_ATTR = "_is_logger_internal"

# Attribute signalling whether the handler has a custom loglevel
COMMON_INTERNAL_HANDLER_IS_CUSTOM_LOGLEVEL = "_is_logger_internal_handler_custom_loglevel"
...
def setup_logger(name=None, logfile=None, level=logging.DEBUG, formatter=None, maxBytes=0, backupCount=0, fileLoglevel=None, disableStderrLogger=False):
    ...
    .. code-block:: python
        ...
        from algoproto.common import setup_logger
        logger = setup_logger()
        logger.info("hello")
    ...
    for handler in list(_logger.handlers):
        if hasattr(handler, COMMON_INTERNAL_LOGGER_ATTR):
    ...
    if disableStderrLogger:
        ...
        setattr(stderr_stream_handler, COMMON_INTERNAL_LOGGER_ATTR, True)
    ...
    if logfile:
        ...
        setattr(rotating_filehandler, COMMON_INTERNAL_LOGGER_ATTR, True)
...
class LogFormatter(logging.Formatter):
    ...
    * Robust against str/bytes encoding problems.
    ...
    DEFAULT_DATE_FORMAT = '%m%dT%H:%M:%S'
    DEFAULT_COLORS = {
...
def _stderr_supports_color():
    ...
    if os.getenv('LOGGER_FORCE_COLOR') == '1':
        return True
...
def reset_default_logger():
    ...
    _logfile = None
    _formatter = None
    logger = setup_logger(name=COMMON_DEFAULT_LOGGER, logfile=_logfile, level=_loglevel, formatter=_formatter)
...
def loglevel(level=logging.DEBUG, update_custom_handlers=False):
    """
    Set the minimum loglevel for the default logger (`common.logger`).
    ...
    for handler in list(logger.handlers):
        if hasattr(handler, COMMON_INTERNAL_LOGGER_ATTR) or update_custom_handlers:
            # Don't update the loglevel if this handler uses a custom one
            if hasattr(handler, COMMON_INTERNAL_HANDLER_IS_CUSTOM_LOGLEVEL):
                continue
...
def formatter(formatter, update_custom_handlers=False):
    """
    Set the formatter for all handlers of the default logger (``common.logger``).
...
def logfile(filename, formatter=None, mode='a', maxBytes=0, backupCount=0, encoding=None, loglevel=None, disableStderrLogger=False):
    ...
    if filename:
        ...
        setattr(rotating_filehandler, COMMON_INTERNAL_LOGGER_ATTR, True)
        if loglevel:
            setattr(rotating_filehandler, COMM_INTERNAL_HANDLER_IS_CUSTOM_LOGLEVEL, True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # print exception
        _logger.error('except', exc_info=True)
    _logger.debug('world')
