...
{
    public:
    std::string code; // 商品代码 AP，br，bu 等; 特殊代码 CS: Common Stock, 即股票;  ETF: Exchange Traded Fund, 即交易所交易基金; LOF Listed Open-Ended Fund，即上市型开放式基金; INDX: Index, 即指数; FT: Futures，即期货，包含股指、国债和商品期货; OP: Option 期权
...
public:
    ...
    std::string name;
    // 交易所
    std::string exchange; // SZ,SH,HK, DCE,CZCE,SHFE,CFFEX,INE
    // 最小价格变动单位
    double tick_size;
...
public:
    ...
    int64_t buy(const Symbol& symbol, int64_t qty, double price);
    int64_t sell(const Symbol& symbol, int64_t qty, double price);

    int64_t buy_open(const Symbol& symbol, int64_t qty, double price);
    int64_t sell_open(const Symbol& symbol, int64_t qty, double price);
    int64_t buy_close(const Symbol& symbol, int64_t qty, double price);
    int64_t sell_close(const Symbol& symbol, int64_t qty, double price);
...
class CerebroEventSink
{
    public:
    void on_market_open();
    void on_pre_market_settle(); // 触发broker处理结算事务
    void on_pre_market_open();
    void on_market_close();
    void on_aft_market_close();
    void on_aft_market_settle(); // 触发broker处理结算事务
    void on_tick(); // 加载tick缓存， 触发用户业务回调on_tick，再触发broker撮合
    void on_kline(); // 根据时间点加载k线缓存，再处理用户业务回调on_kline
};
