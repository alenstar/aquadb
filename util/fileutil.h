...
enum {
    ...
    D_GAMESETTINGS_IDX,    // user-specified settings which override both the
                        // global and the default settings (per game)
    D_MAPS_IDX,
    D_CACHE_IDX,
    D_SHADERCACHE_IDX,
    D_SHADERS_IDX,
...
// The functions in this class are functionally identical to the standalone
// functions below, but if you are going to be calling more than one of the
// functions using the same path, creating a single FileInfo object and calling
// its functions multiple times is faster than calling standalone functions
// multiple times.
...
public:
    ...
    bool IsFile() const;
    // Returns the size of a file (or returns 0 if the path doesn't refer to a
    // file)
    u64 GetSize() const;
