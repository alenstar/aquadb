...
public:
    ...
    Interface* createObject( const std::string& className )
    {
        ...
        {
            std::cerr << " ObjectFactoryMgr::createObject className " << className << " empty! " << std::endl;
            return NULL;
        }
    }
    ...
    static ObjectFactoryMgr< Interface >& instance()
    {
        static ObjectFactoryMgr< Interface > gObjectFactoryMgr;
...
//构造函数不带参数的反射
#define CREATE_REFLECTION_OBJECT(BaseClass, name) ObjectFactoryMgr<BaseClass>::instance().createObject((name))

#define DECLAR_REFLECTION_CLASS(BaseClass, ConcreteClass) \
	static BaseClass* ConcreteClass##createObj(){ return new ConcreteClass;}\
	static BaseClass* ConcreteClass##Tmp = ObjectFactoryMgr<BaseClass>::instance().reg(#ConcreteClass, ConcreteClass##createObj);

//构造函数带一个字符串带参数的反射
#define CREATE_REFLECTION_OBJECT_WITH_PARAM(BaseClass, name,param) ObjectFactoryMgr<BaseClass>::instance().createObjectWithParam((name) ,param)
#define DECLAR_REFLECTION_CLASS_WITH_PARAM(BaseClass, ConcreteClass) \
	...
	static BaseClass* ConcreteClass##Tmp = ObjectFactoryMgr<BaseClass>::instance().regWithParam(#ConcreteClass, ConcreteClass##createObj);
