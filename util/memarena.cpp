...
static int AshmemCreateFileMapping(const char *name, size_t size)
{
    ...
    if (ret < 0) {
        close(fd);
        LOGW("Ashmem returned error: 0x%08x", ret);
        return ret;
    }
    return fd;
}
...
void MemArena::GrabSHMSegment(size_t size)
{
    ...
    const std::string name =
        "dolphin-emu." + std::to_string(GetCurrentProcessId());
    hMemoryMapping =
        CreateFileMapping(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0,
                          static_cast<DWORD>(size), UTF8ToTStr(name).c_str());
#elif defined(ANDROID)
    fd = AshmemCreateFileMapping(
        ("dolphin-emu." + std::to_string(getpid())).c_str(), size);
    if (fd < 0) {
        LOGW("Ashmem allocation failed");
        return;
    }
#else
    ...
    if (ftruncate(fd, size) < 0) LOGE("Failed to allocate low memory space");
#endif
}
...
{
#ifdef _WIN32
    return MapViewOfFileEx(hMemoryMapping, FILE_MAP_ALL_ACCESS, 0,
                           (DWORD)((u64)offset), size, base);
#else
    void *retval =
...
u8 *MemArena::FindMemoryBase()
{
    ...
    u8 *base = static_cast<u8 *>(
        VirtualAlloc(nullptr, memory_size, MEM_RESERVE, PAGE_READWRITE));
    if (!base) {
        LOGE("Failed to map enough memory space: %s",
