                   ...
                   std::function<bool(const File::FSTEntry &)> callback)
{
    ...
    for (const std::string &directory : directories) {
        ...
        DoEntry = [&](File::FSTEntry &entry) {
            if (callback(entry)) result.push_back(entry.physicalName);
...
DoFileSearch(const std::vector<std::string> &directories,
             const std::vector<std::string> &exts, bool recursive)
{
    bool accept_all = exts.empty();
    return FileSearchWithTest(
        directories, recursive, [&](const File::FSTEntry &entry) {
            ...
            return std::any_of(
                exts.begin(), exts.end(), [&](const std::string &ext) {
                    ...
                    return name.length() >= ext.length() &&
                           strcasecmp(name.c_str() + name.length() -
                                          ext.length(),
                                      ext.c_str()) == 0;
                });
        });
             ...
             const std::vector<std::string> &exts, bool recursive)
{
    ...
    auto ext_matches = [&native_exts](const fs::path &path) {
        ...
        return std::any_of(
            native_exts.cbegin(), native_exts.cend(),
            [&native_path](const auto &ext) {
                // TODO provide cross-platform compat for the comparison
                // function, once more platforms support std::filesystem
                int compare_len = static_cast<int>(ext.native().length());
                return native_path.length() >= compare_len &&
                       CompareStringOrdinal(
                           &native_path
                                .c_str()[native_path.length() - compare_len],
                           compare_len, ext.c_str(), compare_len,
                           TRUE) == CSTR_EQUAL;
            });
    };
    ...
    for (const auto &directory : directories) {
        if (recursive) {
            ...
            for (auto &entry :
                 fs::recursive_directory_iterator(fs::u8path(directory)))
                add_filtered(entry);
        }
        else {
    ...
    // Remove duplicates (occurring because caller gave e.g. duplicate or
    // overlapping directories - not because std::filesystem returns
    // duplicates). Also note that this pathname-based uniqueness isn't as
    // thorough as std::filesystem::equivalent.
    std::sort(result.begin(), result.end());
    result.erase(std::unique(result.begin(), result.end()), result.end());
