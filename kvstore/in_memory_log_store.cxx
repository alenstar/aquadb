...
ulong inmem_log_store::next_slot() const {
    ...
    LOGD("local_idx: %lu, start_idx: %lu", static_cast<uint64_t>(logs_.size() -1), static_cast<uint64_t>(start_idx_));
...
ptr<log_entry> inmem_log_store::last_entry() const {
    ...
    LOGD("index:%lu, term:%lu, val_type:%d", next_idx-1, entry->second->get_term(), static_cast<int>(entry->second->get_val_type()));
    ...
    inmem_log_store::log_entries(ulong start, ulong end)
{
    ...
    LOGD("index:%lu, term:%lu, val_type:%d", ii, entry->second->get_term(), static_cast<int>(entry->second->get_val_type()));
            src = entry->second;
        }
...
ptr<log_entry> inmem_log_store::entry_at(ulong index) {
    ...
    LOGD("index:%lu, term:%lu, val_type:%d", index, entry->second->get_term(), static_cast<int>(entry->second->get_val_type()));
        src = entry->second;
    }
...
ptr<buffer> inmem_log_store::pack(ulong index, int32 cnt) {
    ...
    LOGD("index:%lu, term:%lu, val_type:%d", ii, le->get_term(), static_cast<int>(le->get_val_type()));
        assert(le.get());
...
void inmem_log_store::apply_pack(ulong index, buffer& pack) {
    ...
    LOGD("index:%lu, term:%lu, val_type:%d", cur_idx, le->get_term(), static_cast<int>(le->get_val_type()));
        }
    }
